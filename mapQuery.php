...
try{
    ...
    $sth = $pdo->prepare("SELECT distinct x,y FROM history where id = :id and day=1");
    $sth->bindValue(':id', $_POST['ino'], PDO::PARAM_INT);
    $sth->execute();
    $smap = $sth->fetch(PDO::FETCH_ASSOC);
    $originalX = $smap['x'];
    $originalY = $smap['y'];
}catch(Exception $e){
    syslog(LOG_ERR,'mapQuery '.$e->getMessage());
}


    //syslog(LOG_INFO,"ino=".$_POST['ino']);
    //syslog(LOG_INFO,"originalX=".$originalX);
    //syslog(LOG_INFO,"originalY=".$originalY);
    ...
    for( $i = 0 ; $i < $dx-1 ; $i++){
        ...
        for ( $j = 0; $j < $dy -1; $j+=1  ){
            ...
            if ( $kx+$i == $lx and $ky+$j*2+$sss == $ly) {
                $filcol = ImageColorAllocate($img, $r, 200, $b);
            }
