<?php
    ...
    while(True){        // 無限ループに注意
        ...
        $x = $myx;
        $y = $myy;
        do_mission($pdo,$myteam,$x,$y,$myimpact,$rate);
        ...
        // 現地の情報を再取得する
        [$enemy,$r,$b] = check_here($pdo,$mytF,$myx,$myy);
...
function check_here($pdo,$t,$x,$y){
    ...
    // 会敵チェック
    $sth = $pdo->prepare("SELECT * FROM angels where (x = :x and y = :y) and not (team = :t)");
    $sth->bindValue(':t', $t, PDO::PARAM_STR);
    $sth->bindValue(':x', $x, PDO::PARAM_INT);
    $sth->bindValue(':y', $y, PDO::PARAM_INT);
    $sth->execute();
    $ene = $sta->fetch(PDO::FETCH_ASSOC);
    if ($sth->rowCount()==0){
        $enemy = 0;
    }else{
        $enemy = $ene['fight'];
